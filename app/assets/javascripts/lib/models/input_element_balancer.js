...
(function(window) {
  ...
  Balancer.get = function(name, options) {
    if (!Object.prototype.hasOwnProperty.call(Balancer.balancers, name)) {
  ...
  Balancer.prototype.doBalance = function(newValue, quinn) {
    ...
    var sLength = this.subordinates.length;
    ...
    var valuesBeforeBalancing = new OriginalValues(this.subordinates);
    ...
    //
    var flex = this.oValues.sumOf(this.subordinates) + newValue;
    ...
    for (var i = 0; i < sLength; i++) {
      ...
      var slider = this.subordinates[i];
      var prevValue = this.oValues.value(slider);
  ...
  Balancer.prototype.balanceToClosestEquilibrium = function(quinn, target) {
    ...
    var lower = isIncreasing ? quinn.previousValue : target;
    var upper = isIncreasing ? target : quinn.previousValue;
    var knownGood;
    var previous;

    for (;;) {
      var midPoint = this.snapValue((lower + upper) / 2);
  ...
  Balancer.prototype.isMaster = function(quinn) {
    return this.masterId !== null && this.masterId === quinn.balanceId;
  };
  ...
  Balancer.prototype.begin = function(value, quinn) {
    ...
    // Otherwise it is a "begin" event being fired when a subordinate slider is
  ...
  Balancer.prototype.__getSubordinates = function() {
    ...
    var subs = _.select(this.quinns, function(quinn) {
  ...
  Balancer.prototype.__runOnSubordinates = function(method) {
    var subLength = this.subordinates.length;
  ...
  OriginalValues.prototype.revert = function() {
    for (var i = 0; i < this.length; i++) {
      this.quinns[i].setTentativeValue(this.values[this.quinns[i].balanceId]);
    }
  };
