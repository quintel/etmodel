class AddWattNuView < ActiveRecord::Migration
  def self.up
    watt_nu = RootNode.create :key => 'watt_nu'
    Tab.all.each do |tab|
     puts tab.key
     tab_node = TabNode.create(:element => tab, :type => 'TabNode', :parent => watt_nu)
  ...
  def self.down
    ViewNode.find_by_key('watt_nu').destroy
  end
end
