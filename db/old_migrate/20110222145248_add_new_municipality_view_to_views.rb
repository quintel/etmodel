class AddNewMunicipalityViewToViews < ActiveRecord::Migration
  def self.up
    new_municipality_view = RootNode.create :key => 'new_municipality_view'
    Tab.all.each do |tab|
      puts tab.key
      tab_node = TabNode.create(:element => tab, :type => 'TabNode', :parent => new_municipality_view)
  ...
  def self.down
    ViewNode.find_by_key('new_municipality_view').destroy
  end
end