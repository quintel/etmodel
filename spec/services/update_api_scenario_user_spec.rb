...
describe UpdateAPIScenarioUser, type: :service do
  let(:client) do
    Faraday.new do |builder|
      builder.adapter(:test) do |stub|
        stub.put('/api/v3/scenarios/123/users') do |_env|
          response
        end
      end
    end
  ...
  let(:scenario_user) { { id: 1, email: nil, role: 'scenario_collaborator' } }
  ...
  context 'when the response is successful' do
    let(:response) do
      [
        ...
        { id: 1, email: nil, role: 'scenario_collaborator' }
      ]
    end
  ...
  context 'when the response is unsuccessful' do
    ...
    it 'returns the scenario error messages' do
      expect(result.errors).to eq(['Failed to update scenario user: '])
    end
  end
end
