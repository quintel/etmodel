...
null&&!this.start())return false}else if(a.type==="keyup"&&this.previousValue!=null)this.options.keyFloodWait?this.keyFloodTimer=window.setTimeout(g.bind(function(){this.resolve()},this),this.options.keyFloodWait):this.resolve();switch(a.which){case 33:this.stepUp(10,false,true);break;case 34:this.stepDown(10,false,true);break;case 37:case 40:a.altKey?this.setTentativeValue(this.model.minimum,false):this.stepDown(a.shiftKey?10:1,false,true);break;case 39:case 38:a.altKey?this.setTentativeValue(this.model.maximum,
false):this.stepUp(a.shiftKey?10:1,false,true);break;default:return true}a.preventDefault()};j.prototype.setValue=function(a,b){var d=this.values,c,f,a=g.isArray(a)?g.clone(a):[a];for(f=0,c=a.length;f<c;f++)a[f]=this.sanitizeValue(a[f],b);if(g.isEqual(a,d))return false;this.value=this.values=a;if(this.values.length===1)this.value=a[0];return this.value};j.prototype.roundToStep=function(a){var b=1/this.step,b=Math.round(a*b)/b;g.isArray(this.only)&&(b=g.min(this.only,function(b){return Math.abs(b-
